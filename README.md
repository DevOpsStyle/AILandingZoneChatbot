...
3. **Networking** — CIDR VNet + subnet PE + subnet APIM.
4. **AI Foundry & modelli** — selettore "**numero di modelli**" (0–3). Per ogni modello attivo: `deploymentName`, `modelName`, `format` (OpenAI/Microsoft/Meta/Mistral/Cohere/DeepSeek), `version`, `skuName`, `capacity`. Per ogni modello viene creato un **backend APIM** corrispondente (`aoai-<deploymentName>`) puntato all'endpoint privato di Foundry.
5. **API Management** — publisher email/name.
6. **Diagnostica** — master switch + toggle per singolo servizio (Foundry, Search, APIM, Cosmos, Service Bus, Redis, Key Vault).
7. **Defender for Cloud** — master switch + toggle per piano (AiServices, Api, CosmosDbs, KeyVaults, Storage, Containers, Arm, VM, AppServices, SQL, SQL VM, OSS DB).
8. **Tag** — workload, env, owner.
...
| API              | API Management (StandardV2, public access disabled, Private Endpoint) **+ 1 backend per modello** (`aoai-<deploymentName>`) |
| Data             | Azure Cosmos DB (DocumentDB, SQL API)                                   |
| Messaging        | Azure Service Bus (Premium)                                             |
| Cache            | Azure Managed Redis (Redis Enterprise — `Balanced_B5`)                  |
| Secrets          | Azure Key Vault (RBAC, public access disabled) — *aggiunto come dipendenza necessaria per gestire chiavi/secret in modo sicuro* |
| Networking       | VNet + subnet PE + subnet APIM + NSG + 9 Private DNS Zones + VNet Links |
| Observability    | Log Analytics Workspace + Diagnostic Settings su tutte le risorse       |
| Security         | Microsoft Defender for Cloud (subscription-scope, plan-by-plan)         |

Tutti i servizi sono **`publicNetworkAccess: Disabled`** e raggiungibili solo via **Private Endpoint** verso le rispettive Private DNS Zones.
...
└── deployment: workload-deploy        (RG scope)
    ├── Log Analytics
    ├── NSG + VNet + Subnets
    ├── 9 Private DNS Zones + VNet Links
    ├── Key Vault
    ├── Foundry + 2 model deployments
    ├── AI Search
    ├── APIM (StandardV2)
    ├── Cosmos DB
    ├── Service Bus Premium
    ├── Redis Enterprise + database
    ├── 7 Private Endpoints + DNS Zone Groups
    └── 7 Diagnostic Settings (condizionali)
```

Le dipendenze sono modellate esplicitamente con `dependsOn` per garantire ordine di creazione corretto (es. PE dopo VNet+servizio, DNS group dopo PE+zona, diagnostiche dopo risorsa+LAW).
...
- `locations` (object): override **per singolo componente**. Lascia `""` per usare il default.
  ...
  ```
  - `network` = region della VNet, NSG **e di tutti i Private Endpoint** (i PE devono stare nella stessa region della VNet).
...
>
> **Resource Group**: di default viene **creato**. Per usarne uno esistente imposta `useExistingResourceGroup: true` e `resourceGroupName` al nome del RG già presente.
...
- **APIM StandardV2**: supporta Private Endpoint nativo (non usa VNet injection). La subnet APIM viene creata ed è disponibile se in futuro si vuole passare a Premium classic con injection.
